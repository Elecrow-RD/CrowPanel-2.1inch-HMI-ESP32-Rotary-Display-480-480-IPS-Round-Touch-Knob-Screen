...
static void apply_image_arc_angle_mask(lv_opa_t * mask_buf, int32_t abs_x, int32_t abs_y, int32_t len,
                                       int32_t center_x, int32_t center_y, int32_t start_angle, int32_t end_angle);
...
void lv_draw_sw_arc(lv_draw_unit_t * draw_unit, const lv_draw_arc_dsc_t * dsc, const lv_area_t * coords)
{
    ...
    if(dsc->img_src == NULL) {
        ...
        /*
         * RGB565A8 stores the alpha plane after the RGB565 pixels. Arc drawing
         * already needs mask_buf for its angle/radius mask, therefore it cannot
         * pass the image alpha plane to the blender as a second mask. Fold the
         * image alpha into the arc mask scanline below and blend the color plane
         * as RGB565. This keeps SquareLine's transparent segmented arc images
         * working without changing their generated data.
         */
    ...
    for(h = 0; h < blend_h; h++) {
        ...
        if(dsc->img_src && decoder_dsc.decoded->header.cf == LV_COLOR_FORMAT_RGB565A8) {
            /*Apply the inner/outer ring masks here. The image-arc angle mask is
             *calculated explicitly below so the selected sector is not inverted.*/
            void * angle_mask = mask_list[0];
            mask_list[0] = NULL;
            blend_dsc.mask_res = lv_draw_sw_mask_apply(mask_list, mask_buf, blend_area.x1, blend_area.y1, blend_w);
            mask_list[0] = angle_mask;
        }
        else {
            blend_dsc.mask_res = lv_draw_sw_mask_apply(mask_list, mask_buf, blend_area.x1, blend_area.y1, blend_w);
        }
        ...
        if(dsc->img_src && decoder_dsc.decoded->header.cf == LV_COLOR_FORMAT_RGB565A8) {
            /*
             * LVGL 9.1's generic angle-mask path selects the complementary
             * sector for image arcs that cross 0 degrees (e.g. 135 -> 175 is
             * displayed as the other 320 degrees). Rebuild only that angular
             * part from the descriptor angles, then combine it with the image
             * alpha below. This makes the visible colored segments track the
             * actual arc value while leaving normal color arcs untouched.
             */
            apply_image_arc_angle_mask(mask_buf, blend_area.x1, blend_area.y1, blend_w,
                                       dsc->center.x, dsc->center.y, start_angle, end_angle);

            int32_t img_y = blend_area.y1 - img_area.y1;
            int32_t img_x1 = blend_area.x1 - img_area.x1;
            int32_t alpha_stride = decoder_dsc.decoded->header.stride / 2;
            const lv_opa_t * alpha_plane = (const lv_opa_t *)decoder_dsc.decoded->data +
                                           decoder_dsc.decoded->header.stride * decoder_dsc.decoded->header.h;

            int32_t x;
            for(x = 0; x < blend_w; x++) {
                int32_t img_x = img_x1 + x;
                lv_opa_t img_opa = 0;
                if(img_y >= 0 && img_y < decoder_dsc.decoded->header.h && img_x >= 0 &&
                   img_x < decoder_dsc.decoded->header.w) {
                    img_opa = alpha_plane[img_y * alpha_stride + img_x];
                }
                mask_buf[x] = LV_OPA_MIX2(mask_buf[x], img_opa);
            }
                                       ...
                                       int32_t center_x, int32_t center_y, int32_t start_angle, int32_t end_angle)
{
    ...
    int32_t x;
    for(x = 0; x < len; x++) {
        int32_t dx = abs_x + x - center_x;
        int32_t dy = abs_y - center_y;
        if(dx == 0 && dy == 0) {
            mask_buf[x] = 0;
            continue;
        }
        int32_t angle = (int32_t)lv_atan2(dy, dx);
        int32_t delta = angle - start_angle;
        if(delta < 0) delta += 360;

        if(delta > sweep) mask_buf[x] = 0;
    }
}
